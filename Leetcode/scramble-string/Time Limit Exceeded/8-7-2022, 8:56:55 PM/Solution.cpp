...
class Solution {
private:
    int util(string &s1, string &s2, int x1, int x2, int y1, int y2, vector<vector<vector<vector<int>>>> &dp) {
        int n = x2-x1+1;
...
public:
    bool isScramble(string s1, string s2) {
        int n = s1.length();
        vector<vector<vector<vector<int>>>> dp(30, vector<vector<vector<int>>> (30, vector<vector<int>> (30, vector<int> (30, -1))));
